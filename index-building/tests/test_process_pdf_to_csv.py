...
class TestProcessPdfToCsv:
    ...
    def test_process_pdf_creates_csv_with_headers(self):
        ...
        try:
            ...
            with patch('generate_chunks.create_gemini_client', return_value=mock_client):
                with patch('generate_chunks.upload_pdf', return_value=mock_uploaded_file):
                    with patch('generate_chunks.generate_with_fallback', return_value=mock_response):
        ...
        # Check headers
        assert reader.fieldnames == ['description', 'section_name', 'query']
    ...
    def test_process_pdf_appends_to_existing_csv(self):
        ...
        with open(csv_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=['description', 'section_name', 'query'])
            writer.writeheader()
            writer.writerow({
    ...
    def test_process_pdf_missing_api_key(self):
        ...
        with patch('generate_chunks.create_gemini_client', side_effect=ValueError("GOOGLE_API_KEY not found in environment variables")):
    ...
    def test_process_pdf_file_not_found(self):
        ...
        with patch('generate_chunks.create_gemini_client', return_value=mock_client):
            with patch('generate_chunks.upload_pdf', side_effect=FileNotFoundError("PDF file not found")):
    ...
    def test_process_pdf_all_models_fail(self):
        ...
        with patch('generate_chunks.create_gemini_client', return_value=mock_client):
            with patch('generate_chunks.upload_pdf', return_value=mock_uploaded_file):
                with patch('generate_chunks.generate_with_fallback', side_effect=RuntimeError("All models failed")):
