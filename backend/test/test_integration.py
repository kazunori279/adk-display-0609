...
async def mock_client(server):
    ...
    with patch('app.main.start_agent_session') as mock_start_session:
        ...
        mock_live_events = AsyncIteratorMock([
            MockEvent(content=MockContent(parts=[MockPart(text="Hello!")]), partial=True),
        ...
        mock_start_session.return_value = (mock_live_events, mock_live_request_queue)
...
class MockEvent:
    ...
    def __init__(self, content=None, partial=False, turn_complete=False, interrupted=False):
...
@pytest.mark.asyncio
async def test_server_startup_and_shutdown(server):
    """Test that the server starts up and responds to requests."""
    async with httpx.AsyncClient() as client:
        # Test that the server is running and responds to the root endpoint
        response = await client.get(f"{server.base_url}/")
        assert response.status_code == 200


@pytest.mark.asyncio
async def test_text_message_communication(mock_client):
    """Test sending and receiving text messages."""
    # Send a text message
    response = await mock_client.send_text_message("Hello, server!")
    ...
    assert len(mock_client.events_received) >= 0  # May receive mock events


@pytest.mark.asyncio
async def test_audio_message_communication(mock_client):
    """Test sending and receiving audio messages."""
    # Create dummy audio data
    audio_data = b"fake_audio_data_for_testing"
    
    # Send an audio message
    response = await mock_client.send_audio_message(audio_data)
    assert response["status"] == "sent"
    ...
    assert len(mock_client.events_received) >= 0
...
async def test_multiple_clients(server):
    ...
    try:
        ...
        with patch('test.test_integration.start_agent_session') as mock_start_session:
            mock_live_events = AsyncIteratorMock([
                MockEvent(turn_complete=True, interrupted=False)
            ])
            mock_live_request_queue = MagicMock()
            ...
            for i in range(3):
                client = MockClient(server, user_id=i+1)
                await client.connect_sse()
